...
from utils import CoM, get_data, fixOverlap, addAtoms, sortR0,sortProj

def do_nothing(parent):
    """
    Do nothing
    """
    clus = parent
...
def skin(parent):
    ...
    clus = addAtoms(core, eleNames, eleNums)
    clus = fixOverlap(clus)
...
def changeCore(parent):
    ...
    if inout == 1:
        ...
        clus = sortR0(clus, R0)
        del clus[iout]
        clus = addAtoms(clus, eleNames, eleNums)
        clus = fixOverlap(clus)

    return clus


def mate(parent1, parent2, surfGA=False):
    """
    ...
    while compositionWrong:
        ...
        #cmax = fit1/(fit1 + fit2)
        cut = np.abs(int(natoms * ran.uniform(0.2, 0.8)))
        if cut == 0:
            cut = 1
        elif cut == natoms:
            cut = natoms - 1
