...
from mutations import do_nothing, homotop, rattle_mut, rotate_mut, twist, partialInversion, tunnel, skin, changeCore
...
class MCSEnv(gym.Env):
	...
	"""
        ...
        clus = addAtoms(clusm, self.eleNames, self.eleNums)
        ...
        with open("initial_clus_gen.txt", "a+") as fh:
            fh.write(f"counter: {counter} mutation: {mut} ene_before_mut: {ene_before_mut} ene_after_mut: {ene_after_mut}  \n")
